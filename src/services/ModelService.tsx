...
const MODEL_IDS = {
  llm: 'qwen3-1.7b-q4_k_m', // Qwen3 1.7B - native tool-call format, small enough to never jetsam
...
// Resolves once registerDefaultModels() has finished. The chat screen awaits
// this before trying to load the LLM, so it never races ahead of registration
// (which is delayed in dev builds while Metro lazily bundles the backends).
let resolveModelsReady: (() => void) | null = null;
export const modelsReady: Promise<void> = new Promise(resolve => {
  resolveModelsReady = resolve;
});
...
export const ModelServiceProvider: React.FC<ModelServiceProviderProps> = ({ children }) => {
  ...
  const downloadAndLoadLLM = useCallback(async () => {
    ...
    try {
      // Wait until models are registered (avoids "Model not found" race in dev).
      await modelsReady;
...
export const registerDefaultModels = async () => {
  // LLM Model - Qwen3 1.7B (Q4_K_M, ~1.1GB). The 4B Q4_K_M (~2.5GB weights +
  // KV cache) jetsammed an iPhone 14 Pro Max, so we run the smallest Qwen3
  // that still handles the Hermes-style tool-call decision pass reliably.
  // Hybrid thinking model — prompts must include /no_think or it emits
  // <think> blocks (handled in AgentService).
  ...
  // Signal that all models are registered so the UI can safely load them.
  resolveModelsReady?.();
};
